...
if __name__ == '__main__':
	...
	mred = control.control('control.ini',base_directory, red = True)

	ipdb.set_trace()


	mred.calibrate_ao('MRED')

#	rv_control.backlight(mred)
	

	# test backlight fiber identification
#	camera = mred.cameras[0]
#	images = glob.glob('/Data/mred/n20200114/n202001??.MRED.FAU.backlight.03??.fits')
#	for imagename in images:
#		print imagename, rv_control.find_fiber(imagename, camera)

#	mred.raster(mred.telescopes[0].id, dwelltime=120.0, stepsize=3)
	
#	newauto.autofocus(mred,'MRED')

#	ipdb.set_trace()

#	telescope = utils.getTelescope(mred,'MRED')
...
#	mred.telescopes[0].makePointingModel(mred, npoints = 50)
#	ipdb.set_trace()
#	ipdb.set_trace()
#	ipdb.set_trace()

#	mred.doSkyFlat(['ip'],False,1,mred.telescopes[0].id,piggyback=False)
#	ipdb.set_trace()

	#mred.getPA('/Data/mred/n20171114/n20171114.MRED.Ross248.ip.0239.fits')
	target = {
		"name": "HD62613", 
		"ra": 23.25, 
		"dec": 45.75, 
...
#	target['ra'] = mred.telescopes[0].lst() # make sure it's up
#	mred.night = 'n20200107'
#	rv_control.doSpectra(mred,target,['MRED'],simulate=True)
...
#	t0 = datetime.datetime.utcnow()
	mred.takeFauImage(target,'MRED')
#	print (datetime.datetime.utcnow() - t0).total_seconds()
	ipdb.set_trace()

#	mred.telescopes[0].calibrateRotator(mred.cameras[0])

	ipdb.set_trace()
#	mred.cameras[0].take_image(exptime=5,filterInd='ip',piggyback=True)
#	ipdb.set_trace()

	mred.telescopes[0].makePointingModel(mred,npoints=50,exptime=0.2)
#	minerva.endNight(num=3,email=False,kiwispec=False)

#	newauto.autofocus(minerva,'TRED')
...
#		"dec": 80.26554, 
	...
	newauto.autofocus(minerva,'TRED',target=target)
	ipdb.set_trace()
#	rv_control.acquireFocusGuide(minerva,target,'TRED')
#	minerva.takeFauImage(target,'TRED')
#	ipdb.set_trace()
...
#	minerva.telescopes[1].makePointingModel(minerva,npoints=100,exptime=2.0)
	...
	minerva.takeSpectrum(target)
	ipdb.set_trace()
	newauto.autofocus(minerva,'TRED',target=target,defocus_step=0.03,num_steps=3,dome_override=True)
	ipdb.set_trace()
	minerva.takeSpectrum(target,['TRED'])
...
#	'''	
	...
	newauto.autofocus(minerva,'TRED',fau=True,target=target)
	"""
	target = {
		"name" : "HD125455", 
		"ra" : 20.5,#14.3263513,
...
#	filename = minerva.takeImage(target,telid)
