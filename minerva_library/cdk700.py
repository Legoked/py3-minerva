...
class CDK700:
    def __init__(
        ...
        if directory == None:
            if red:
                self.directory = "directory_red.txt"
            elif south:
                self.directory = "directory_south.txt"
            else:
                self.directory = "directory.txt"
        else:
    ...
    def guessFocus(self, m3port):
        ...
        try:
            ambtemp = float(status.temperature.ambient)
        except:
            ambtemp = m1temp - float(self.dT0[m3port])
        try:
            gravity = math.sin(float(status.mount.alt_radian))
        except:
            gravity = float(self.G0[m3port])
        dt = m1temp - ambtemp
        self.focus[m3port] = (
            float(self.F0[m3port])
        ...
        self.logger.info(
            "Setting nominal focus for port "
            + m3port
            + " at "
            + str(self.focus[m3port])
            + " when T="
            + str(m1temp)
            + ", dT="
            + str(dt)
    ...
    def isInitialized(self, tracking=True, derotate=True):
        ...
        if telescopeStatus.mount.alt_enabled != "True":
            self.logger.info(
                "altitude motor not enabled ("
                + telescopeStatus.mount.alt_enabled
                + "), telescope not initialized"
            )
            return False
        if telescopeStatus.mount.alt_motor_error_message != "No error":
            self.logger.info(
                "altitude motor error present ("
                + telescopeStatus.mount.alt_motor_error_message
                + "), telescope not initialized"
            )
            return False
        if telescopeStatus.mount.azm_enabled != "True":
            self.logger.info(
                "azimuth motor not enabled ("
                + telescopeStatus.mount.azm_enabled
                + "), telescope not initialized"
            )
            return False
        if telescopeStatus.mount.azm_motor_error_message != "No error":
            self.logger.info(
                "azimuth motor error present ("
                + telescopeStatus.mount.azm_motor_error_message
                + "), telescope not initialized"
            )
            return False
        if telescopeStatus.mount.connected != "True":
            self.logger.info(
                "mount not connected ("
                + telescopeStatus.mount.connected
                + "), telescope not initialized"
            )
            return False
        if telescopeStatus.rotator.connected != "True":
            self.logger.info(
                ...
                + telescopeStatus.rotator.connected
                + "), telescope not initialized"
            )
            return False
        if telescopeStatus.focuser.connected != "True":
            self.logger.info(
                ...
                + telescopeStatus.focuser.connected
                + "), telescope not initialized"
            )
            return False
        ...
        if tracking:
            if telescopeStatus.mount.tracking != "True":
                self.logger.info(
                    "mount not tracking ("
                    + telescopeStatus.mount.tracking
                    + "), telescope not initialized"
                )
                return False
        else:
        ...
        if derotate:
            if telescopeStatus.rotator.altaz_derotate != "True":
                self.logger.info(
                    ...
                    + telescopeStatus.rotator.altaz_derotate
                    + "), telescope not initialized"
                )
                return False
        else:
    ...
    def initialize(self, tracking=True, derotate=True):
        ...
        if (
            telescopeStatus.mount.alt_enabled != "True"
            or telescopeStatus.mount.azm_enabled != "True"
        ):
        ...
        if (
            telescopeStatus.focuser1.connected != "True"
            or telescopeStatus.rotator1.connected != "True"
        ):
            self.logger.info("Connecting to focuser")
            if not self.focuserConnect("1"):
                return False
            time.sleep(5.25)
            telescopeStatus = self.getStatus()
        ...
        # connect to the focuser if not connected
        if (
            telescopeStatus.focuser2.connected != "True"
            or telescopeStatus.rotator2.connected != "True"
        ):
            self.logger.info("Connecting to focuser")
            if not self.focuserConnect("2"):
                return False
            time.sleep(5.25)
            telescopeStatus = self.getStatus()
    ...
    def load_config(self):
        try:
            ...
            try:
                self.minfocus = config["MINFOCUS"]
            except:
                self.minfocus = {"0": 100, "1": 100, "2": 100}
            try:
                self.maxfocus = config["MAXFOCUS"]
            except:
                self.minfocus = {"0": 33000, "1": 33000, "2": 33000}
            try:
                self.win10 = config["Setup"]["WIN10"]
            except:
                self.win10 = False
        ...
        for key in self.focus_offset:
            try:
                self.focus_offset[key] = float(self.focus_offset[key])
            except:
                pass
    ...
    def getStatus(self):
        ...
        if xmlstatus == False:
            ...
            status = self.parseXml(xmlstatus)
        if status == False:
            xmlfile = open(
                self.base_directory + "/dependencies/telstateunknown.xml", "r"
            )
        ...
        self.logger.debug(
            "Alt/Az RMS error: "
            + status.mount.alt_rms_error_arcsec
            + ","
            + status.mount.azm_rms_error_arcsec
        )
        self.logger.debug(
            "Alt/Az: " + status.mount.alt_radian + "," + status.mount.azm_radian
        )
        return status
    ...
    def getFocuserStatus(self, m3port):
        telescopeStatus = self.getStatus()
        if str(m3port) == "1":
            return telescopeStatus.focuser1
        return telescopeStatus.focuser2

    def getRotatorStatus(self, m3port, telescopeStatus=None):
        if telescopeStatus == None:
            telescopeStatus = self.getStatus()
        if str(m3port) == "1":
            return telescopeStatus.rotator1
    ...
    def focuserMove(self, position, m3port):
        ...
        if position < float(self.minfocus[m3port]) or position > float(
            self.maxfocus[m3port]
        ):
            self.logger.warning(
                "Requested focus on port "
                + m3port
                + " ("
                + str(position)
                + ") out of bounds"
        ...
        return self.pwiRequestAndParse(
            device="focuser" + str(m3port), cmd="move", position=position
        )
    ...
    def focuserMoveAndWait(self, position, m3port, timeout=300.0):
        ...
        if m3port == 1:
            lock = self.focuser1_lock
        else:
            lock = self.focuser2_lock
        lock.acquire()
        ...
        if not self.focuserMove(position, m3port):
            self.logger.warning(
                "Focuser on port "
                + str(m3port)
                + " could not move to requested position ("
                + str(position)
                + ")"
            )
            lock.release()
    ...
    def focuserIncrement(self, offset, m3port):
        ...
        return self.pwiRequestAndParse(
            device="focuser" + str(m3port), cmd="move", increment=offset
        )
    ...
    def focuserHomeAndWait(self, m3port, timeout=300.0):
        ...
        focuserStatus = self.getFocuserStatus(m3port)
        if not focuserStatus.finding_home == "True":
            self.focuserHome(m3port)
        ...
        self.logger.info(
            "Homing focuser "
            + str(m3port)
            + " complete; moving to nominal position (elapsed time = "
            + str(elapsedTime)
            + ")"
        )
        ...
        while abs(float(focuserStatus.position) - 1000) > 10 and elapsedTime < 20.0:
            ...
            self.logger.info(
                "Moving focuser "
                + str(m3port)
                + " to nominal position (elapsed time = "
                + str(elapsedTime)
                + ")"
            )
            time.sleep(1.0)
    ...
    def rotatorMove(self, position, m3port, wait=False, timeout=400.0):
        if m3port == 1:
            lock = self.rotator1_lock
        else:
            lock = self.rotator2_lock
        lock.acquire()

        status = self.pwiRequestAndParse(
            device="rotator" + str(m3port), cmd="move", position=position
        )
        if not wait:
            lock.release()
            return status
    ...
    def rotatorIncrement(self, offset, m3port):
        return self.pwiRequestAndParse(
            device="rotator" + str(m3port), cmd="move", increment=offset
        )
    ...
    def rotatorStartDerotating(self, m3port):
        return self.pwiRequestAndParse(
            device="rotator" + str(m3port), cmd="derotatestart"
        )

    def rotatorStopDerotating(self, m3port):
        return self.pwiRequestAndParse(
            device="rotator" + str(m3port), cmd="derotatestop"
        )
    ...
    def rotatorHomeAndWait(self, m3port, timeout=400.0):
        ...
        # make sure it's not homing in another thread first
        rotatorStatus = self.getRotatorStatus(m3port)
        if not rotatorStatus.finding_home == "True":
            self.rotatorHome(m3port)
    ...
    def parangle(self, target=None, useCurrent=False, status=None):
        ...
        return (
            ...
            / math.pi
            * math.atan2(
                -math.sin(ha * math.pi / 12.0),
                math.cos(dec * math.pi / 180.0)
                * math.tan(float(self.site.latitude) * math.pi / 180.0)
    ...
    def solveRotatorPosition(self, target):
        ...
        while rotator_pos < 0.0:
            rotator_pos += 360.0

        rotator_pos = math.fmod(rotator_pos, 360.0)
        self.logger.info(
            "Calculated a mech position of "
            + str(rotator_pos)
            + " for PA of "
            + str(desiredPA)
        )
    ...
    def mountEnableMotors(self):
        ...
        if status.mount.azm_enabled == "False" or status.mount.alt_enabled == "False":
            ...
            status = self.pwiRequestAndParse(device="mount", cmd="enable")
            if (
                status.mount.azm_enabled == "False"
                or status.mount.alt_enabled == "False"
            ):
    ...
    def mountOffsetRaDec(self, deltaRaArcseconds, deltaDecArcseconds):
        return self.pwiRequestAndParse(
            device="mount",
            cmd="move",
            incrementra=deltaRaArcseconds,
            incrementdec=deltaDecArcseconds,
        )
    ...
    def mountOffsetAltAz(self, deltaAltArcseconds, deltaAzArcseconds):
        return self.pwiRequestAndParse(
            device="mount",
            cmd="move",
            incrementazm=deltaAzArcseconds,
            incrementalt=deltaAltArcseconds,
        )
    ...
    def mountOffsetAltAzFixed(self, deltaAltArcseconds, deltaAzArcseconds):
        ...
        t0 = datetime.datetime.utcnow()
        slewtimeAlt = (
            2.0  # slewtimeAz - (datetime.datetime.utcnow() - t0).total_seconds()
        )
        self.pwiRequestAndParse(
            device="mount",
            cmd="jog",
    ...
    def mountGotoRaDecApparent(self, raAppHours, decAppDegs):
        ...
        return self.pwiRequestAndParse(
            device="mount", cmd="move", ra=raAppHours, dec=decAppDegs
        )
    ...
    def mountSetPointingModel(self, filename):
        return self.pwiRequestAndParse(
            device="mount", cmd="setmodel", filename=filename
        )

    def mountAddToModel(self, ra, dec):
        return self.pwiRequestAndParse(
            device="mount", cmd="addtomodel", ra2000=ra, dec2000=dec
        )

    def mountSaveModel(self, filename):
        return self.pwiRequestAndParse(
            device="mount", cmd="savemodel", filename=filename
        )

    def mountSync(self, ra, dec):
        return self.pwiRequestAndParse(
            device="mount", cmd="sync", ra2000=ra, dec2000=dec
        )
    ...
    def recover(self, tracking=True, derotate=True):
        ...
        self.logger.warning(
            "failed " + str(self.nfailed) + " times; trying to reconnect"
        )
        ...
        body = (
            ...
            + "I have failed to recover automatially. Please recover me, then delete "
            + filename
        ...
        while not self.initialize(tracking=tracking, derotate=derotate):
            self.logger.error(
                "Telescope has failed to automatically recover; intervention required"
            )
            mail.send(
                self.id + " has failed", body, level="serious", directory=self.directory
            )
            fh = open(filename, "w")
            fh.close()
    ...
    def focusVsAlt(self, minerva, npoints=25, minalt=30, maxalt=85):
        brightstars = utils.brightStars(filename="bsc_rand.csv")
        nstars = len(brightstars["dec"])

        foci = np.zeros(npoints)
        alts = np.zeros(npoints)
        success = np.zeros(npoints)

        ndx = 0
        for i in range(npoints):
            alt = -999

            while alt < minalt or alt > maxalt:
            ...
            if autofocus(minerva, self.id, exptime=1.0, target=target):
                success[i] = 1

            status = self.getStatus()
            m3port = status.m3.port
            focus = self.focus[m3port]

            foci[i] = focus
            alts[i] = alt

        data = np.transpose([alts, foci, success])
        np.savetxt(
            self.datadir + minerva.site.night + "/alt_focus." + self.id + ".txt", data
        )
    ...
    def makePointingModel(
        ...
        if brightstar:
            ...
            if shuffle:
                random.shuffle(shufflendx)
        else:
            self.logger.error(
                "brightstar, random, or grid must be set to make a pointing model"
            )
        ...
        while pointsAdded < npoints:
            ...
            if minerva.fauguide(
                ...
                # add point to model
                self.logger.info(
                    "Adding point to model: ra = " + str(ra) + ", dec = " + str(dec)
                )
            ...
            p = subprocess.Popen(
                ["xy2sky", datapath + imageName, str(xcenter), str(ycenter)],
...
#			deccen2 = dec - decoffset/3600.0
            ...
            # add point to model
            self.logger.info(
                "Adding point to model: RA_Center = "
                + str(racen)
                + ", Dec_center = "
                + str(deccen)
                + ", ra = "
    ...
    def calibrateRotator(self, camera, fau=True, exptime=1):
        ...
        self.logger.info(
            "Enter the sky position angle (" + str(skypa) + ") into the calibrate field"
        )
    ...
    def inPosition(
        ...
        if (str(m3port) == "1") or (str(m3port) == "2"):
            ...
            if elapsedTime > timeout:
                self.logger.error(
                    "Failed to select correct M3 port (" + str(m3port) + ")"
                )
                return False
        ...
        # S If a bad port is specified (e.g. 3) or no port (e.g. None)
            ...
            self.logger.info(
                "No M3 port specified or bad, using current port ("
                + telescopeStatus.m3.port
                + ")"
            )
            m3port = telescopeStatus.m3.port

        self.logger.info(
            "Waiting for telescope to finish slew; moving = "
            + telescopeStatus.mount.moving
            + str(telescopeStatus.mount.moving == "True")
            + str(elapsedTime < timeout)
            + str(tracking)
        )
        timeout = 60.0
        while (
            telescopeStatus.mount.moving == "True"
            and elapsedTime < timeout
            and tracking
        ):
            time.sleep(0.25)
            elapsedTime = (datetime.datetime.utcnow() - start).total_seconds()
        ...
        focuserStatus = self.getFocuserStatus(m3port)
        self.logger.info(
            "Waiting for Focuser to finish slew; goto_complete = "
            + focuserStatus.goto_complete
        )
        timeout = 300.0
        ...
        if elapsedTime > timeout:
            self.logger.error(
                "Failed to get to focus position within timeout (" + str(timeout) + ")"
            )
            return False
        ...
        if alt != None and az != None:
            ...
            DeltaPos = (
                math.acos(
                    math.sin(ActualAlt) * math.sin(alt * math.pi / 180.0)
                    + math.cos(ActualAlt)
                    * math.cos(alt * math.pi / 180.0)
                    * math.cos(ActualAz - az * math.pi / 180.0)
                )
                * (180.0 / math.pi)
                * 3600.0
            )
            if DeltaPos > pointingTolerance:
                self.logger.error(
                    "Telescope reports it is "
                    + str(DeltaPos)
                    + " arcsec away from the requested postion (ActualAlt="
                    + str(ActualAlt * 180.0 / math.pi)
                    + " degrees, Requested Alt="
                    + str(alt)
                    + ", ActualAz="
                    + str(ActualAz * 180.0 / math.pi)
                    + " degrees, Requested Az="
                    + str(az)
                )
                self.nfailed += 1
                return False
        ...
        if ra != None and dec != None:
            ...
            DeltaPos = (
                math.acos(
                    math.sin(ActualDec) * math.sin(dec * math.pi / 180.0)
                    + math.cos(ActualDec)
                    * math.cos(dec * math.pi / 180.0)
                    * math.cos(ActualRa - ra * math.pi / 12.0)
                )
                * (180.0 / math.pi)
                * 3600.0
            )
            if DeltaPos > pointingTolerance:
                self.logger.error(
                    ...
                    + str(ActualDec * 180.0 / math.pi)
                    + " degrees, Requested Dec="
                    + str(dec)
                    + ", RA="
                    + str(ActualRa * 12.0 / math.pi)
                    + " hours, Requested Ra="
                    + str(ra)
                )
                self.nfailed += 1
        ...
        if derotate:
            ...
            timeout = 360.0
            self.logger.info(
                "Waiting for rotator to finish slew; goto_complete = "
                + rotatorStatus.goto_complete
            )
            while rotatorStatus.goto_complete == "False" and elapsedTime < timeout:
                time.sleep(0.5)
                self.logger.debug(
                    "rotator moving (" + str(rotatorStatus.position) + " degrees)"
                )
                elapsedTime = (datetime.datetime.utcnow() - start).total_seconds()
                rotatorStatus = self.getRotatorStatus(m3port)
        ...
        if elapsedTime > timeout:
            self.logger.error(
                "Failed to get to rotator position within timeout ("
                + str(timeout)
                + ")"
            )
            return False
    ...
    def starmotion(self, ra, dec, pmra, pmdec, px=0.0, rv=0.0, date=None):
        ...
        year_sec = days_in_year * 24.0 * 3600.0  # [] = seconds/year
        # S Parsecs in an AU
        pctoau = 3600.0 * 180 / math.pi  # [] = AU
        # S km/sec to AU/year
        kmstoauy = year_sec * 1000.0 / AU
        ...
        r0hat = np.array(
            [
                np.cos(rarad) * np.cos(decrad),
                np.sin(rarad) * np.cos(decrad),
                np.sin(decrad),
            ]
        )
        ...
        mu = (pmra * east + pmdec * north) / pctoau / 1000.0
        ...
        vpi = (rv / 1000.0) * kmstoauy * (px / 1000.0 / pctoau)
        ...
        if ra_intermed < 0:
            ra_corrected = np.degrees(ra_intermed + 2 * np.pi) / 15.0
        else:
            ra_corrected = np.degrees(ra_intermed) / 15.0
    ...
    def acquireTarget(self, target, pa=None, tracking=True, derotate=True, m3port=None):
        ...
        if tracking == None:
            ...
            self.logger.info(
                "tracking not requested, but is required for acquisition; turning on"
            )
            tracking = True
        ...
        if target["spectroscopy"]:
            if derotate == None:
                ...
                self.logger.info(
                    "derotation requested but should not be used for spectra; turning off"
                )
                derotate = False
            ...
            if m3port == None:
                ...
                self.logger.info(
                    "Imaging port requested, but FAU is required for spectroscopy; using FAU port"
                )
                m3port = self.port["FAU"]
            ...
            if m3port == None:
                ...
                self.logger.info(
                    "FAU port requested, but IMAGER is required for imaging; using imaging port"
                )
                m3port = self.port["IMAGER"]
        ...
        ra_corrected, dec_corrected = self.starmotion(
            target["ra"], target["dec"], pmra, pmdec, px=px, rv=rv
        )
        ...
        if alt < self.horizon or alt > 85:
            self.logger.error(
                "Coordinates out of bounds; object not acquired! (Alt,Az) = ("
                + str(alt)
                + ","
                + str(az)
                + "), (RA,Dec) = ("
        ...
        self.logger.info(
            "Starting slew to J2000 " + str(ra_corrected) + "," + str(dec_corrected)
        )
        if self.isMoving():
            self.logger.error(
                "Telescope moving when another move requested, aborting acquireTarget"
            )
            return False
        ...
        if not self.isInitialized(tracking=tracking, derotate=derotate):
            self.logger.error(
                "probably tracked into limits; waiting to slew, then re-initializing"
            )
            time.sleep(10)
            # S make sure the m3 port is in the correct orientation and it's tracking
            self.initialize(tracking=tracking, derotate=derotate)
        ...
        if self.inPosition(
            ...
            self.logger.info(
                "Finished slew to J2000 " + str(ra_corrected) + "," + str(dec_corrected)
            )
            return True
        else:
            self.logger.error(
                "Slew failed to J2000 " + str(ra_corrected) + "," + str(dec_corrected)
            )
            ...
            return self.acquireTarget(
                target, pa=pa, tracking=tracking, derotate=derotate, m3port=m3port
            )
    ...
    def m3port_switch(self, m3port, force=False):
        ...
        if (str(m3port) == "1") or (str(m3port) == "2"):
            ...
            if telescopeStatus.m3.port != str(m3port) or force:
                ...
                if os.path.isfile(modelfile):
                    ...
                    self.logger.error(
                        "model file (%s) does not exist; using current model"
                        % (modelfile)
                    )
                    mail.send(
                        "model file (%s) does not exist; using current model"
        ...
        # S If a bad port is specified (e.g. 3) or no port (e.g. None)
            ...
            self.logger.error(
                "Bad M3 port specified (%s); using current port(%s)"
                % (m3port, telescopeStatus.m3.port)
            )
    ...
    def park(self, parkAlt=25.0, parkAz=0.0):
        ...
        self.logger.info(
            "Parking telescope (alt=" + str(parkAlt) + ", az=" + str(parkAz) + ")"
        )
        ...
        if not self.inPosition(
            alt=parkAlt, az=parkAz, pointingTolerance=3600.0, derotate=False
        ):
            if self.recover(tracking=False, derotate=False):
                self.park()
    ...
    def recoverFocuser(self, focus, m3port):        # havent tested w/ py3; should belong in higher level code
        ...
        mail.send(
            self.id + " port " + str(m3port) + " focuser failed",
            "",
            level="serious",
            directory=self.directory,
        )
    ...
    def home(self, timeout=600):  # 420.0):
        ...
        telescopeStatus = self.getStatus()
        if not telescopeStatus.mount.is_finding_home == "True":
            self.mountHome()

        # make sure it's connected
        if not telescopeStatus.mount.connected == "True":
            self.logger.warning(
                "Home requested but not connected; reconnecting telescope"
            )
        ...
        while telescopeStatus.mount.is_finding_home == "True" and elapsedTime < timeout:
            elapsedTime = (datetime.datetime.utcnow() - t0).total_seconds()
            self.logger.info(
                "Homing Telescope (elapsed time = " + str(elapsedTime) + ")"
            )
            time.sleep(5.0)
            telescopeStatus = self.getStatus()
            if (
                not telescopeStatus.mount.connected == "True"
                or not telescopeStatus.mount.azm_enabled == "True"
                or not telescopeStatus.mount.alt_enabled == "True"
            ):
                self.logger.error(
                    "Motors no longer enabled; reinitializing. Commutation problem?"
                )
                if not self.initialize(tracking=False, derotate=False):
                    self.logger.error("Cannot connect to mount")
                    return False
        ...
        if telescopeStatus.mount.encoders_have_been_set == "False":
            self.logger.error(
                "Error homing telescope (elapsed time = " + str(elapsedTime) + ")"
            )
            return False
        else:
            self.logger.info(
                "Done homing telescope (elapsed time = " + str(elapsedTime) + ")"
            )
            return True
    ...
    def send_to_computer(self, cmd):
        ...
        if "NT_STATUS_HOST_UNREACHABLE" in out:
            ...
            mail.send(
                "Invalid credentials for " + self.id,
                "Dear Benevolent Humans,\n\n"
                + "The credentials in "
                + self.base_directory
