...
# matplotlib.use("Agg", warn=False) : deprecated line, try using : matplotlib.use("Agg") instead
import datetime
import logging
import ephem
import time
...
# import urllib2 # to remove dependancy
...
class site:
    ...
    def load_config(self):
        ...
        self.observing = True
        self.weather = -1
        self.rainChangeDate = datetime.datetime.utcnow() - datetime.timedelta(
            hours=25.0
        )
        self.lastRain = 0.0
        self.mailSent = False
        ...
        self.startNightTime = datetime.datetime(
        ...
        self.openLimits = {
            "totalRain": [0.0, 999999.0],
            "wxt510Rain": [0.0, 99999.0],
            "barometer": [0, 2000],
            ...
            "MearthCloud": [
                -999,
                openCloudLimit * cloudScale["mearth"][0] + cloudScale["mearth"][1],
            ],
            # 			'MearthCloud'         : [-999, -38],
            "HATCloud": [
                -999,
                openCloudLimit * cloudScale["hat"][0] + cloudScale["hat"][1],
            ],
            #'AuroraCloud'         : [-999, openCloudLimit*cloudScale['aurora'][0]    +cloudScale['aurora'][1]],
            "AuroraCloud": [-999, -32],
            "MINERVACloud": [
                -999,
                openCloudLimit * cloudScale["minerva"][0] + cloudScale["minerva"][1],
            ],
            "cloudDate": [
        ...
        self.closeLimits = {
            ...
            "MearthCloud": [
                -999,
                closeCloudLimit * cloudScale["mearth"][0] + cloudScale["mearth"][1],
            ],
            # 			'MearthCloud'         : [-999, -37],
            "HATCloud": [
                -999,
                closeCloudLimit * cloudScale["hat"][0] + cloudScale["hat"][1],
            ],
            # 			'AuroraCloud'         : [-999, closeCloudLimit*cloudScale['aurora'][0]    +cloudScale['aurora'][1]],
            "AuroraCloud": [-999, -31],
            "MINERVACloud": [
                -999,
                closeCloudLimit * cloudScale["minerva"][0] + cloudScale["minerva"][1],
            ],
            "cloudDate": [
                datetime.datetime.utcnow() - datetime.timedelta(minutes=6),
                datetime.datetime(2200, 1, 1),
    ...
    """
    def getWeatherMearth(self): # urllib2 dependancy
        ...
        try:
            ...
            self.logger.error(
                "Error reading the weather page: " + str(sys.exc_info()[0])
            )
            return {}
    ...
    def getWeatherRidge(self):
        ...
        # 		request = urllib2.Request(url)
        try:
            f = open(self.base_directory + "/credentials/webpage.txt")
    ...
    def getWeather(self):
        ...
        with self.lock:
            if self.logger_name == "site_mtHopkins":
                # 2022-10-20 JDE: MEarth weather station down, don't bother checking
                ...
                try:
                    ...
                    if len(cloudstr) == 6:
                        ...
                        if weather["AuroraCloud"] == 0.0:
                            weather["AuroraCloud"] = 999
                        weather["MINERVACloud"] = float(cloudstr[4])
                        if weather["MINERVACloud"] == 0.0:
                            weather["MINERVACloud"] = 999
                        weather["HATCloud"] = 999
                    elif len(cloudstr) == 4:
                        ...
                        weather["HATCloud"] = 999
                        weather["MINERVACloud"] = 999
                        weather["MearthCloud"] = 999  # MEarth disabled as of 2022-02-03
                    else:
                        self.logger.error(
                            "Error reading the cloud page; line is: "
                            + " ".join(cloudstr)
                        )
                    ...
                    self.logger.error(
                        "Error reading the page for cloud temps at " + url
                    )
                    pageError = True
            elif (
                self.logger_name == "site_Simulate"
                or self.logger_name == "site_Wellington"
            ):
        ...
        for key in requiredKeys:
            if not key in weather.keys():
                ...
                logging.error(
                    "Weather page does not have all required keys (" + key + ")"
                )
                pageError = True
    ...
    def getSkyLimit(self, timeout=3.0):
        cmd = self.base_directory + "/minerva_library/getskylimit.sh"
        proc = Popen([cmd], stdout=PIPE, stderr=PIPE)
        try:
            stdout, stderr = proc.communicate()
            return float(stdout.split("=")[1].replace('"', ""))
        except:
            return False
    ...
    def oktoopen(self, domeid, domeopen=False, ignoreSun=False):
        ...
        if os.path.exists(decisionFile):
            ...
            try:
                date = datetime.datetime.strptime(
                    f.readline().strip(), "%Y-%m-%d %H:%M:%S.%f"
                )
            except:
                date = datetime.datetime.utcnow() - datetime.timedelta(days=1.1)
            f.close()

            if (datetime.datetime.utcnow() - date).total_seconds() > 86400.0:
                if not self.mailSent:
                    mail.send(
                        "Possible snow/ice on enclosures; manual inspection required",
                        "Dear benevolent humans,\n\n"
                        + "Recent conditions have been wet and cold ("
                        + str(self.coldestTemp)
                        + " C), which means ice and/or snow is likely. "
                        ...
                        + "). Note that this warning will be tripped again 24 hours after the "
                        + "date in that file.\n\n"
                        "Love,\nMINERVA",
                        level="serious",
                    )
        ...
        if self.mailSent:
            mail.send(
                "Snow/ice conditions have been manually checked and OK'ed",
        ...
        self.sunOverride = (
            os.path.exists(
                self.base_directory + "/minerva_library/sunOverride." + domeid + ".txt"
            )
            or ignoreSun
        )
        self.cloudOverride = os.path.exists(
            self.base_directory + "/minerva_library/cloudOverride." + domeid + ".txt"
        )
        self.timeoutOverride = os.path.exists(
            self.base_directory + "/minerva_library/timeoutOverride." + domeid + ".txt"
        )

        if self.sunOverride:
            weatherLimits["sunAltitude"] = [-90, 90]
        else:
            weatherLimits["sunAltitude"] = [-90, 6]
        ...
        if self.cloudOverride:
            ...
            # JDE 2022-10-20: Automatically update sky limit from FLWO48 observers
            # leave commented because of connection flakiness
            # 			sky_limit = self.getSkyLimit()
            # 			if sky_limit:
            # 				weatherLimits['MearthCloud'] = sky_limit
            # 				weatherLimits['HATCloud'] = sky_limit
            # 				weatherLimits['AuroraCloud'] = sky_limit
            # 				weatherLimits['MINERVACloud'] = sky_limit
        ...
        if (datetime.datetime.utcnow() - self.rainChangeDate).total_seconds() < 1800.0:
            self.logger.info(
                "Not OK to open: it last rained at "
                + str(self.rainChangeDate)
                + ", which is less than 1 hour ago"
            )
            retval = False

        # if it has (or might have) snowed in the last 24 hours, we need manual approval to open
        if (
            (datetime.datetime.utcnow() - self.rainChangeDate).total_seconds() < 86400.0
            and self.coldestTemp < 1.0
        ) or os.path.exists(decisionFile):
            if os.path.exists(decisionFile):
                f = open(decisionFile, "r")
                date = datetime.datetime.strptime(
                    f.readline().strip(), "%Y-%m-%d %H:%M:%S.%f"
                ...
                if (datetime.datetime.utcnow() - date).total_seconds() > 86400.0:
                    with open(decisionFile, "w") as fh:
                        ...
                        self.logger.info(
                            "Not OK to open: there has been precipitation in the last 24 hours and it has been freezing. Manual inspection for snow/ice required"
                        )
                        return False
                else:
                    self.logger.info(
                        "There has been precipitation in the last 24 hours and it has been freezing, but it has been manually approved to open until "
                        + str(date)
                    )
        ...
        if self.weather["MearthCloud"] != 999:
            key = "MearthCloud"
            if (
                self.weather[key] < weatherLimits[key][0]
                or self.weather[key] > weatherLimits[key][1]
            ):
                self.logger.info(
                    "Not OK to open: "
                    + key
                    + "="
                    + str(self.weather[key])
                    + "; Limits are "
                    + str(weatherLimits[key][0])
                    + ","
                    + str(weatherLimits[key][1])
                )
                retval = False
        ...
        if retval:
            self.logger.debug("OK to open")
        return retval

    def sunrise(self, horizon=0, start=None):
        if start == None:
            start = self.startNightTime
        self.obs.horizon = str(horizon)
        sunrise = self.obs.next_rising(
            ephem.Sun(), start=start, use_center=True
        ).datetime()
        return sunrise
    ...
    def NautTwilBegin(self, horizon=-8):
        self.obs.horizon = str(horizon)
        NautTwilBegin = self.obs.next_rising(
            ephem.Sun(), start=self.startNightTime, use_center=True
        ).datetime()
        return NautTwilBegin

    def NautTwilEnd(self, horizon=-12):
        self.obs.horizon = str(horizon)
        NautTwilEnd = self.obs.next_setting(
            ephem.Sun(), start=self.startNightTime, use_center=True
        ).datetime()
        return NautTwilEnd
...
if __name__ == "__main__":
    ...
    cal_time = (b_time + d_time + sf_time) / 3600.0

    nauttwils = []

    for today in dates:
        test_site.obs.date = today
        test_site.startNightTime = datetime.datetime(
            today.year, today.month, today.day, 17
        ) - datetime.timedelta(days=1)
        nauttwils.append((test_site.NautTwilEnd() - today).total_seconds())
    nauttwils = np.array(nauttwils) / 3600.0
    print(
        "fewest minutes between cals and twil: "
        + str((np.min(nauttwils) - cal_time) * 60.0)
    )
    plt.plot(nauttwils)
