...
import ipdb
import logging
import unicodecsv  # used one; decide if we need to import at all
import json
import mail
...
def killmain(red=False, south=False):
    for pid in psutil.pids():
        ...
        if p.name() == "python" and pid != os.getpid():
            if len(p.cmdline()) > 1:
                if (
                    (p.cmdline())[1] == "mainNew.py"
                    or (p.cmdline())[1] == "main.py"
                    or (p.cmdline())[1] == "daytimespec.py"
                ):
                    if len(p.cmdline()) == 3:
...
def readcsv(filename):
    ...
    with open(filename, "rb") as f:
        ...
        for key in csv.keys():
            try:
                csv[key] = np.asarray(csv[key], dtype=np.float32)
            except:
                csv[key] = np.asarray(csv[key])

        return csv


def brightStars(
    filename="bsc.csv", path="/home/minerva/minerva-control/dependencies/", maxmag=6.0
):
...
def parseTarget(line, logger=None):
    ...
    target["starttime"] = datetime.datetime.strptime(
        target["starttime"], "%Y-%m-%d %H:%M:%S"
    )
    target["endtime"] = datetime.datetime.strptime(
        target["endtime"], "%Y-%m-%d %H:%M:%S"
    )
    return target
...
def scheduleIsValid(scheduleFile, email=True, logger=None, directory=None):
    ...
    with open(scheduleFile, "r") as targetfile:
        ...
        if CalibInfo == -1:
            ...
            emailbody = (
                emailbody + "Line " + str(linenum) + ": malformed JSON: " + line + "\n"
            )
        else:
            requiredKeys = [
                "nbias",
            ...
            for key in requiredKeys:
                if key not in CalibInfo.keys():
                    if logger != None:
                        logger.error(
                            "Line 1: Required key (" + key + ") not present: " + line
                        )
                    emailbody = (
                        emailbody
                        + "Line 1: Required key ("
        ...
        if CalibEndInfo == -1:
            ...
            for key in requiredKeys:
                if key not in CalibEndInfo.keys():
                    if logger != None:
                        logger.error(
                            "Line 2: Required key (" + key + ") not present: " + line
                        )
                    emailbody = (
                        emailbody
                        + "Line 2: Required key ("
        ...
        for line in targetfile:
                            ...
                            if target["ra"] > 24.0 or target["ra"] < 0:
                                if logger != None:
                                    logger.error(
                                        ...
                                        + ": RA ("
                                        + str(target["ra"])
                                        + ") must be in decimal hours (0 < RA < 24)"
                                    )
                                emailbody = (
                                    emailbody
                                    + "Line "
                            ...
                            if target["dec"] > 90.0 or target["dec"] < -90:
                                if logger != None:
                                    logger.error(
                                        ...
                                        + ": Dec ("
                                        + str(target["dec"])
                                        + ") must be in decimal degrees (-90 < RA < 90)"
                                    )
                                emailbody = (
                                    emailbody
                                    + "Line "
            ...
            if emailbody != "":
                if email:
                    mail.send(
                        "Errors in target file: " + scheduleFile,
                        emailbody,
                        level="serious",
                        directory=directory,
                    )
...
def truncate_observable_window(
    site, target, sunalt=-12.0, horizon=21.0, timeof=None, logger=None
):
    if timeof == None:
    ...
    while sunrise < timeof and niter < 100:
        sunrise = site.sunrise(
            horizon=sunalt, start=timeof + datetime.timedelta(days=0.1 * niter)
        )
        niter += 1
    if sunrise < timeof:
        if logger != None:
            logger.error("Error calculating nearest sunrise")
        return
    ...
    if sunset > sunrise:
        if logger != None:
            logger.error(
                "Error calculating nearest sunset. Sunset = "
                + str(sunset)
                + " sunrise = "
                + str(sunrise)
            )
    ...
    while risetime > settime and niter < 100:
        try:
            risetime = site.obs.next_rising(
                body, start=sunset - datetime.timedelta(days=0.1 * niter)
            ).datetime()
        except ephem.AlwaysUpError:
...
# def sextract(datapath,imagefile,sexfile='autofocus.20210512.sex',paramfile=None,convfile=None,catfile=None,checktype=None,checkfile=None):
def sextract(
    datapath,
    imagefile,
    sexfile="autofocus.20220503.sex",
    paramfile=None,
    convfile=None,
    catfile=None,
    checktype=None,
    ...
    if checkfile != None:
        sexcommand += (
            " -CHECKIMAGE_TYPE OBJECTS -CHECKIMAGE_NAME " + datapath + checkfile
        )
...
def readsexcat(catname):
    ...
    if np.ndim(cata) > 1:
        cata = cata[
            np.where(cata[:, flag_ind] < 4)
        ]  # Need to revisit this - CD, 20220331
