...
class imager:
    ...
    def load_config(self):
        try:  # common to spectrograph detector and imaging camera
            ...
            try:
                self.telescope_config = config["Setup"]["TELESCOPE_CONFIG"]
            except:
                self.telescope_config = ""

            try:
                self.PBfilters = config["PBFILTERS"]
            except:
                self.PBfilters = None
            try:
                self.PBflatminsunalt = config["PBFLATMINSUNALT"]
            except:
                self.PBflatminsunalt = None

            try:
                self.PBflatmaxsunalt = config["PBFLATMAXSUNALT"]
            except:
                self.PBflatmaxsunalt = None
            try:
                self.PBflattargetcounts = config["PBFLATTARGETCOUNTS"]
            except:
                self.PBflattargetcounts = None
            try:
                self.PBbiaslevel = config["PBBIASLEVEL"]
            except:
                self.PBbiaslevel = None
            try:
                self.PBsaturation = config["PBSATURATION"]
            except:
                self.PBsaturation = None
            try:
                self.PBflatmaxexptime = config["PBFLATMAXEXPTIME"]
            except:
                self.PBflatmaxexptime = None
            try:
                self.PBflatminexptime = config["PBFLATMINEXPTIME"]
            except:
                self.PBflatminexptime = None
    ...
    def cool(self):
        ...
        if currentTemp == "fail":
            ...
            if currentTemp == -999.0:
                self.logger.warning(
                    "The camera failed to connect properly; beginning recovery"
                )
                if self.recover():
                    return self.cool()
                return False
        ...
        lastTimeNotAtTemp = datetime.datetime.utcnow() - datetime.timedelta(
            seconds=oscillationTime
        )
        ...
        while elapsedTime < settleTime and (
            (abs(self.setTemp - currentTemp) > self.maxdiff)
            or elapsedTimeAtTemp < oscillationTime
        ):
            self.logger.info(
                "Current temperature ("
                + str(currentTemp)
            ...
            elapsedTimeAtTemp = (
                datetime.datetime.utcnow() - lastTimeNotAtTemp
            ).total_seconds()
            ...
            if currentTemp == "fail":
                ...
                if (abs(self.setTemp - currentTemp)) > self.maxdiff:
                    self.logger.error(
                        "The camera was unable to reach its setpoint ("
                        + str(self.setTemp)
                        + ") in the elapsed time ("
                        + str(elapsedTime)
                        + " seconds)"
    ...
    def write_status(self):
        ...
        if res[0] == "success":
            ...
            status = open(
                self.base_directory + "/status/" + self.logger_name + ".json", "w"
            )
            status.write(res[1])
            status.close()
    ...
    def simulate_star_image(
        xstar, ystar, flux, fwhm, background=300.0, noise=10.0, guider=True
    ):
    ...
    def getMean(self, guider=False):
        if guider:
            res = self.send("getMean guider", 15).split()
        else:
    ...
    def getMode(self, guider=False):
        if guider:
            res = self.send("getMode guider", 15).split()
        else:
    ...
    def isSuperSaturated(self, guider=False):
        if guider:
            res = self.send("isSuperSaturated guider", 15).split()
        else:
    ...
    def remove(self, guider=False):
        if guider:
            res = self.send("remove guider", 5).split()
        else:
    ...
    def set_binning(self):
        if (
            self.send("set_binning " + str(self.xbin) + " " + str(self.ybin), 5)
            == "success"
        ):
            return True
        else:
    ...
    def set_size(self):
        if (
            self.send(
                "set_size " + self.x1 + " " + self.x2 + " " + self.y1 + " " + self.y2, 5
            )
            == "success"
        ):
            return True
    ...
    def set_temperature(self):
        if self.send("set_temperature " + str(self.setTemp), 5) == "success":
            return True
        else:
            return False
    ...
    def isAOPresent(self):
        ...
        try:
            if (self.send(cmd, 30)).split()[1] == "True":
                return True
        except:
            return False
        return False
    ...
    def homeAO(self):
        cmd = "homeAO None"
        if (self.send(cmd, 30)).split()[0] == "success":
            return True
        return False

    # start exposure
    def expose(
        self, exptime=1, exptype=0, filterInd=None, guider=False, offset=(0.0, 0.0)
    ):
    ...
    def write_header(self, header_info, guider=False):
        ...
        while i < length:
            if self.send("write_header " + header_info[i - 800 : i], 3) == "success":
                ...
                self.logger.error(
                    "Error sending header string::: " + header_info[i - 800 : i]
                )
                return False

        if (
            self.send("write_header_done " + header_info[i - 800 : length] + hdrstr, 10)
            == "success"
        ):
            return True
        else:
    ...
    def getGuideStar(self):
        ...
        xy = utils2.get_stars_cv(
            None, filename=self.datadir + "/" + self.night + "/" + self.guider_file_name
        )
        # JDE 2023-02-06 -- Change centroid algorithm
        # 		xy = utils2.get_stars_brightest(None,filename=self.datadir + '/' + self.night + '/' + self.guider_file_name)
        if len(xy) != 0:
    ...
    def take_image(
        ...
        if ndx == -1:
            ...
            if self.recover():
                return self.take_image(
                    exptime=exptime, filterInd=filterInd, objname=objname, fau=fau
                )
            if fau or piggyback:
                self.guider_file_name = ""
            else:
        ...
        if fau or piggyback:
            filename = self.guider_file_name
        else:
            filename = self.file_name
        self.logger.info("Start taking image: " + filename)
        if filterInd != None:
            filt = filt = self.filters[filterInd]
        else:
            filt = None
    ...
    def compress_data(self, night=None):
        ...
        if night == None:
            night = self.night
        if self.send("compress_data " + night, 30) == "success":
            return True
        else:
            return False
    ...
    def recover_server(self):
        ...
        if self.nserver_failed > 3:
            if not self.mailsent:
                mail.send(
                    self.telid + " server failed",
                    "Please fix and restart mainNew.py",
                    level="serious",
                )
            self.mailsent = True
            sys.exit()
        ...
        if self.nserver_failed > 1:
            self.logger.warning(
                "Server failed more than once; power cycling the camera"
            )
            self.powercycle()
            time.sleep(20)
    ...
    def send_to_computer(self, cmd):
        ...
        if "NT_STATUS_HOST_UNREACHABLE" in out:
            ...
            mail.send(
                "Invalid credentials for " + self.telid,
                "Dear Benevolent Humans,\n\n"
                + "The credentials in "
                + self.base_directory
    ...
    def recover(self):
        ...
        self.logger.warning(
            "Camera failed to recover after restarting maxim; power cycling the camera"
        )
        ...
        filename = (
            self.base_directory + "/minerva_library/imager." + self.telid + ".error"
        )
        while not self.connect_camera():
            mail.send(
                "Camera on " + self.telid + " failed to connect",
